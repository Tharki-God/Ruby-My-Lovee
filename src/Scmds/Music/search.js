...
module.exports = {
    ...
    exec: async (ctx) => {
        ...
        const [query] = args;
        if (!query) return interaction.editReply({ embeds: [util.embed().setDescription("❌ | You Need to Provide Something to Search BRUH.")
            .setFooter(ctx.author.username,  ctx.author.displayAvatarURL({ dynamic: true }))
            .setTimestamp()] });

        try {
            let { tracks } = await music.load(`ytsearch:${query}`);
            if (!tracks.length) return interaction.editReply({ embeds: [util.embed().setDescription(" Couldn't find any results.")
